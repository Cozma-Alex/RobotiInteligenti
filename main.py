    ...
    while True:
        ...
        with lock:
            ...
            if max_contour is not None:
                ...
                # Horizontal tracking
                if abs(x_offset) > 20:  # If target is not centered
                    vel_msg.yaw_rate = -math.radians(
                        x_offset / 100
                    )  # Dividing by 100 to scale down rotation
                else:
                    vel_msg.yaw_rate = 0
            else:
                # Search pattern when no target is found
                vel_msg.x = 0
                vel_msg.yaw_rate = math.radians(20)  # Rotate to search
                pose_msg.pitch = 0
                pose_pub.publish(pose_msg)
                time.sleep(0.2)
